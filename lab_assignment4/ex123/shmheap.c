...
void* shmheap_alloc(shmheap_memory_handle mem, size_t sz) {
    ...
    sem_wait(&(hdlptr->shmheap_mutex));
    if (sz % 8 != 0) { // check for byte alignment of 8
    ...
    while (1) {
        if ((size_t)init->sz == hdlptr->total_size) { //empty heap
        ...
        else if ((size_t)init->sz == 0 && init->last_book_keeper == 0 && init->occupied == 0) {
            init->occupied = 1;
            init->sz = (int)sz;
            init->last_book_keeper = 0;
            hdlptr->used_space = hdlptr->used_space + (size_t)init->sz + sizeof(*init);
        ...
        else if (init->occupied < 1 && init->last_book_keeper == 1) {
            ...
            if ((init->sz - (int)sz) > (int)sizeof(*init)) {
                init->sz = (int)sz;
                book_keeper* header = (book_keeper*)((char*)init + sizeof(*init) + sz);
                header->last_book_keeper = init->last_book_keeper;
                header->occupied = 0;
                header->prev_sz = init->sz;
...
void shmheap_free(shmheap_memory_handle mem, void* ptr) {
    ...
    if (next_header->occupied < 1 && header->last_book_keeper != 1) {
        header->sz = (int)sz + next_header->sz + (int)sizeof(book_keeper);
        if (next_header->last_book_keeper == 1) {
            header->last_book_keeper = 1;
        }
        else {
    ...
    if ((size_t)header->prev_sz > 0) {
        ...
        if (header->last_book_keeper == 1) {
            prev_header->last_book_keeper = 1;
            header->last_book_keeper = 0;
        }
    }
    //sem_post(&shmheap_mutex);
    sem_post(&(hdlptr->shmheap_mutex));
